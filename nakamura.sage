...
# if q is over 2, this is eta_minus_8, cf. page 642 of [Nak04]
def phi0(alpha, q, K): 
    if q.norm() % 4 == 1:
...
# The caracter can be of the form phi0*eta_a * eta_b *eta_c, were a,b are assummed to be odd and c is either -4 or -8 (and it can happen that they are not all present)
# w1 = [a, b] means that the caracter contains eta_a*eta_b
...
# computes eta_l for odd l
def eta_l(alpha, p , K): 
    b = ZZ(alpha.norm())
    return kronecker_symbol(b, p)
...
def eta_b(alpha, b, K): # this is for odd b
    D = K.discriminant()
    a = K.ideal(b).factor()[0][0].gens_reduced()[1] # generates the prime above 2
    f = a.minpoly() # Eisenstein polynomial such that Q_2[x]/(f) = K_b
    f_magma = magma(f)
    bs = ZZ(b).str()
    R = magma('pAdicRing('+bs+',100)')
    OK2 = R.TotallyRamifiedExtension(f_magma) # This is the completion of OK at the prime above 2
    alpha = (magma(alpha.minpoly())).Roots(OK2)[1][1] # we need to view alpha in OK2
    return 1 if alpha.IsSquare() else -1
...
def compute_endomorphism_algebras(field):
    done_over_L = False
    D = field[1] # discriminant of K
    # print '####################'
    # print "Field of Discriminant D = ", D, " = ", D.factor()
    x = QQ['x'].gen()
    K.<r> = NumberField(x^2 - D)
    f = D.factor()
    # first we find the conductor Q of the character phi_0. If there is a q | D which is 3 mod 4 we take Q to be the prime above. If not, we take Q to be the prime above 2 and then phi_0 will be eta_minus_8
    found = False
    for q in f: 
        if q[0] % 4 == 3:
    ...
    x = QQ['x'].gen()
    HH.<a,b> = NumberFieldTower([x^2 - p1s, x^2-p2s])
    x = QQ['x'].gen()
    ...
    a5 = compute_T(LL1, FF1, P1, Q, K, w1 = w1s[0], w2=w2s[0]); b5 = compute_T(LL2, FF2, P2, Q, K,w1 = w1s[0], w2=w2s[0]); # print a5, b5, 'Disc =', QuaternionAlgebra(a5,b5).discriminant(),QuaternionAlgebra(a5,b5).discriminant() == 1; 
    D5 = QuaternionAlgebra(a5,b5).discriminant()
    a6= compute_T(LL1, FF1, P1, Q, K,w1 = w1s[1], w2=w2s[1]); b6 =  compute_T(LL2, FF2, P2, Q, K, w1 = w1s[1], w2=w2s[1]); # print a6, b6,  'Disc =', QuaternionAlgebra(a6,b6).discriminant(), QuaternionAlgebra(a6,b6).discriminant() == 1  ; 
    D6 = QuaternionAlgebra(a6,b6).discriminant()
    a7 = compute_T(LL1, FF1, P1, Q, K, w1 = w1s[2], w2=w2s[2]); b7 = compute_T(LL2, FF2, P2, Q, K, w1 = w1s[2], w2=w2s[2]); #print a7, b7,  'Disc =', QuaternionAlgebra(a7,b7).discriminant(),QuaternionAlgebra(a7,b7).discriminant() == 1; 
    D7 =  QuaternionAlgebra(a7,b7).discriminant()
    a8 = compute_T(LL1, FF1, P1, Q, K, w1 = w1s[3], w2=w2s[3]); b8 = compute_T(LL2, FF2, P2, Q, K, w1 = w1s[3], w2=w2s[3]); # print a8, b8,  'Disc =', QuaternionAlgebra(a8,b8).discriminant(), QuaternionAlgebra(a8,b8).discriminant() == 1; 
...
for i in range(len(data)):
    try:
        compute_endomorphism_algebras(data[i])
